...
//        }
    ...
    public void onDrawFrame(long imageNativeBufferPtr, int format, int width, int height, int stride, int cameraId) {
        if (mMediaChainEngine != null) {
            ...
            Log.d(TAG, "inputAngle=" + inputAngle + " ,outputAngle=" + outAngle + ", flipAxis=" + flipAxis + ", cameraId=" + cameraId + ", width: " + width + ", height: " + height);
            ...
            if (isAlgDataRendered && !USE_FRAME_SYNCHRONIZED) {
                ...
                Log.i(TAG, Thread.currentThread().getId() + " - " + "updateInputNativeBufferAndRunAlg : " + (SystemClock.uptimeMillis() - now) + "ms");
            }
        }
    }
