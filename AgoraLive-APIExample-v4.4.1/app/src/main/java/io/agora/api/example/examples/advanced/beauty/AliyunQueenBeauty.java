...
import io.agora.api.example.databinding.FragmentBeautyAliyunQueenBinding;
import io.agora.api.example.utils.TokenUtils;
import io.agora.beautyapi.aliyunqueen.AliyunQueenBeautyAPI;
import io.agora.beautyapi.aliyunqueen.AliyunQueenBeautyAPIKt;
import io.agora.beautyapi.aliyunqueen.CameraConfig;
import io.agora.beautyapi.aliyunqueen.CaptureMode;
import io.agora.beautyapi.aliyunqueen.Config;
import io.agora.beautyapi.aliyunqueen.EventCallback;
...
/**
 * The type Byte Aliyun Queen beauty.
 */
public class AliyunQueenBeauty extends BaseFragment {
    private FragmentBeautyAliyunQueenBinding mBinding;
    ...
    private final AliyunQueenBeautyAPI aliyunQueenBeautyAPI = AliyunQueenBeautyAPIKt.createAliyunQueenBeautyAPI();

    @Nullable
    @Override
    public View onCreateView(@NonNull LayoutInflater inflater, @Nullable ViewGroup container, @Nullable Bundle savedInstanceState) {
        mBinding = FragmentBeautyAliyunQueenBinding.inflate(inflater, container, false);
...
//        }
        ...
        aliyunQueenBeautyAPI.initialize(new Config(requireContext(), rtcEngine,
                new EventCallback(beautyStats -> null,
                        () -> {
                            AliyunQueenBeautySDK.INSTANCE.initEffect(requireContext());
                            return null;
                        },
                        () -> {
                            AliyunQueenBeautySDK.INSTANCE.unInitEffect();
                            return null;
                        }),
                CaptureMode.Agora,
                0,
                false, new CameraConfig()));
        aliyunQueenBeautyAPI.enable(true);
    }
    ...
    private void initVideoView() {
        ...
        mBinding.smallVideoContainer.setOnClickListener(v -> updateVideoLayouts(!AliyunQueenBeauty.this.isLocalFull));
    }
    ...
    private void updateVideoLayouts(boolean isLocalFull) {
        ...
        if (isLocalFull) {
            ...
            if (mRemoteVideoLayout != null) {
                mRemoteVideoLayout.getChildAt(0).setOnClickListener(v -> updateVideoLayouts(!AliyunQueenBeauty.this.isLocalFull));
            ...
            if (mLocalVideoLayout != null) {
                mLocalVideoLayout.getChildAt(0).setOnClickListener(v -> updateVideoLayouts(!AliyunQueenBeauty.this.isLocalFull));
                ViewParent parent = mLocalVideoLayout.getParent();
                if (parent instanceof ViewGroup && parent != mBinding.smallVideoContainer) {
                    ((ViewGroup) parent).removeView(mLocalVideoLayout);
